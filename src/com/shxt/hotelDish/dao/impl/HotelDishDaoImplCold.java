...
public class HotelDishDaoImplCold implements IHotelDishDao{
	...
	public List<Dishes> selDishes(){
		...
		try{
			...
			String sql="select * from dishes where type='¡π≤À'";
	...
	public List<OrderList> do_dishes_sel_orderlist(){
		...
		try{
			conn=DBUtil.getDBConn();
			stat=conn.createStatement();
			String sql="select * from orderlist where `condition`='Œ¥¥¶¿Ì'";
			rs=stat.executeQuery(sql);
			while(rs.next()){
